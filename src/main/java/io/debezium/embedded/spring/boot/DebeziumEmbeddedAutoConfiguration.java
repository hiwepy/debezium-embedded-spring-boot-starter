...
import io.debezium.embedded.history.SchemaHistoryConfigurer;
import io.debezium.embedded.history.SchemaHistoryConfigurerFactory;
...
@ConditionalOnClass({ io.debezium.engine.DebeziumEngine.class })
...
public class DebeziumEmbeddedAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(value = "debezium.embedded.async", havingValue = "true", matchIfMissing = true)
    public ChangeEventHandler asyncRecordChangeEventHandler(RowDataHandler<List<Map<String, String>>> rowDataHandler,
        ...
        return new DefaultChangeEventHandler(entryHandlerProvider.stream().collect(Collectors.toList()), rowDataHandler, debeziumTaskExecutor);
    }

    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(value = "debezium.embedded.async", havingValue = "false")
    ...
    public DebeziumEmbeddedClient singleDebeziumEmbeddedClient(DebeziumEmbeddedProperties properties,
        ...
        for (DebeziumEmbeddedProperties.Instance instance : properties.getInstances()) {
            ...
            DebeziumSchemaHistoryProperties historyProperties = instance.getHistory();
            ...
            // 4. 创建 DebeziumEngine
            if (DebeziumEmbeddedProperties.EventType.ChangeEvent.equals(instance.getEventType()) && Objects.nonNull(changeEventHandler)) {
                DebeziumEngine.Builder<ChangeEvent<String, String>> engineBuilder = DebeziumEngine
                        .create(Json.class)
                        .using(config.asProperties())
                        .using(clock)
                        .using(instance.getPolicy())
            ...
            } else if (DebeziumEmbeddedProperties.EventType.RecordChangeEvent.equals(instance.getEventType()) && Objects.nonNull(recordChangeEventHandler)) {
